...
export default function Chatbot() {
  ...
  const handleOptionClick = (option: string) => {
    ...
    switch (option) {
      ...
      case 'Thank You':
        addBotMessage("**Thank you for choosing SK Cleaning – Where Perfection is Standard!** ✨\n\nHave a wonderful day!");
  ...
  const restartConversation = () => {
    ...
    addBotMessage("✨ **Welcome back to SK Cleaning – Where Perfection is Standard.**\n\nHow can we help you achieve perfection today?", true, 500);
  };
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'SK Cleaning Leads');

    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    
    saveAs(data, `SK_Cleaning_Leads_${new Date().toISOString().split('T')[0]}.xlsx`);
          ...
          <div className="p-3 sm:p-4 border-t bg-white rounded-b-2xl">
            <div className="flex space-x-2">
              ...
              <Input
                ...
                className={`flex-1 text-sm h-10 rounded-lg border transition-all duration-300 ${
                  waitingForInput
